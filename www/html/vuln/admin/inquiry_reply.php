...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    if (isset($_GET["inquiry_id"]) && !empty(trim($_GET["inquiry_id"]))) {
        ...
        // 過去の回答を取得するSQL文
        // ユーザーからの入力を直接SQL文に埋め込む実装
        $stmt = $pdo->query("SELECT InquiryPost.*, User.username FROM InquiryPost INNER JOIN User ON InquiryPost.user_id = User.id WHERE inquiry_id = $inquiry_id AND InquiryPost.deleted = 0 ORDER BY InquiryPost.created_date ASC");
    ...
    <?php foreach ($responses as $response): ?>
        <div class="card mb-3">
            ...
            <div class="card-footer fw-semibold">
                投稿日時:
                <?php echo $response['created_date']; ?>
