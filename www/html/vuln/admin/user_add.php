...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // 新規ユーザを追加するSQL文
    // ユーザからの入力値を直接受け付ける実装
    $query = "INSERT INTO User (username, password, organization_id, mail_address, is_admin) VALUES ('$name', '$password', '$organization_id', '$mail_address', '$is_admin')";
        ...
        <select class="form-select" id="organization_id" name="organization_id" required>
            <?php
                foreach ($organizations as $org) {
                    echo "<option value='" . $org["id"] . "'>" . $org["orgname"] . "</option>";
                }
            ?>
        </select>
    </div>
