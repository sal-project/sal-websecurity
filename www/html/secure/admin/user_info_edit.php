...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $pdo->prepare("UPDATE User SET username = :name$password_update, organization_id = :organization_id, mail_address = :mail_address, is_admin = :is_admin WHERE id = :user_id");
    $stmt->bindValue(':user_id', $_POST["user_id"]);
    $stmt->bindValue(':name', $_POST["name"]);
    if (!empty($password)) {
        $stmt->bindValue(':password', $password);
    }
    $stmt->bindValue(':organization_id', $_POST["organization_id"]);
...
<form action="./user_info_edit.php" method="post">
    ...
    <div class="mb-3">
        ...
        <select class="form-select" id="organization_id" name="organization_id" required>
            ...
            foreach ($organizations as $org) {
                ...
                echo "<option value='" . htmlspecialchars($org["id"]) . "' $selected>" . htmlspecialchars($org["orgname"]) . "</option>";
            }
            ?>
        </select>
    </div>
    <div class="mb-3">
        ...
        <input type="email" class="form-control" id="mail_address" name="mail_address" value="<?php echo htmlspecialchars($user["mail_address"]); ?>" required>
    </div>
