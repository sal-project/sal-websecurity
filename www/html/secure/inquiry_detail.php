...
if (isset($_GET["inquiry_id"]) && !empty(trim($_GET["inquiry_id"]))) {
    ...
    $stmt = $pdo->prepare("SELECT *
                            ...
                            WHERE Inquiry.id = :inquiry_id AND User.organization_id = :organization_id");
    ...
    // 過去の回答を取得するSQL文
    $stmt = $pdo->prepare("SELECT InquiryPost.*, User.username AS responder_name FROM InquiryPost INNER JOIN User ON InquiryPost.user_id = User.id WHERE inquiry_id = :inquiry_id AND InquiryPost.deleted = 0 ORDER BY InquiryPost.created_date ASC");
    $stmt->bindParam(':inquiry_id', $inquiry_id);
    $stmt->execute();
