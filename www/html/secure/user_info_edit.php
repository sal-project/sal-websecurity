...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_SESSION['user_id'] != intval($user_id)) {
        echo "ユーザIDが一致していません。";
        exit;
    }

    $password_update = !empty($password) ? ", password = :password" : "";
    $stmt = $pdo->prepare("UPDATE User SET username = :name$password_update, mail_address = :mail_address WHERE id = :user_id");
...
<a href="user_info.php?user_id=<?php echo htmlspecialchars($user["id"]); ?>" class="btn btn-secondary mt-3">戻る</a>
